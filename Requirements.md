...
FR3 – Start Game Session
The system shall allow a user (authenticated or guest) to start a new game session.
FR3.1 The system shall create a unique session identifier for each game.
FR3.2 The system shall return the game settings (and a reproducible seed, if used).
...
FR7 – End Game Session and Save Result
The system shall allow the client to submit a completed game result (win/loss, time, difficulty, and session id).
FR7.1 The system shall store game session results in PostgreSQL.
FR7.2 The system shall reject invalid submissions (e.g., unknown session id, missing fields).
